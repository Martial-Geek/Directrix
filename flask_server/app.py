...
def create():
    if request.method == 'POST':
        ...
        todo = user(name=name, email_id=email, phone_number=ph_num, aadhar=aadhar,
                    birth_date=date, blood_grp=grp, weight=weight, height=height)
        db.session.add(todo)
        db.session.commit()
...
def upload_file(sno):
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            path = r"C:\\TestNode\\flask_server\\test_Report\\"
            old = os.path.join(path, filename)
            name = role+"-"+str(todo.sno)+'.pdf'
            new = os.path.join(path, name)
            file.save(old)
            os.rename(old, new)
            test1 = test_report(num=todo.sno, name=role, path=new)
...
class user(db.Model):
    ...
    blood_press = db.Column(db.JSON, default=json.dumps(
        {}, cls=json.JSONEncoder), nullable=True)
    Sugar_lev = db.Column(db.JSON, default=json.dumps(
        {}, cls=json.JSONEncoder), nullable=True)
