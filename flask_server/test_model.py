...
def pred(X):
    model = tf.keras.models.load_model(
        "C:/TestNode/flask_server/best_model.h5")
...
def precaution(X):
    main_df2 = pd.read_csv(r"C:\TestNode\flask_server\symptom_precaution.csv")
...
def description(X):
    main_df3 = pd.read_csv(r"C:\TestNode\flask_server\symptom_Description.csv")
...
def medicine(X, lst):
    df = pd.read_csv(r"C:\TestNode\flask_server\Medication.csv")
    df2 = pd.read_csv(
        r"C:\TestNode\flask_server\directrix_symptoms_database_01.csv")
    test = ''
    med = ''
    dosage = ''
    s_med = []
